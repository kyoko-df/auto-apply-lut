...
use crate::database::runtime::upsert_video_info;
...
pub async fn start_video_processing(
    ...
    let (valid_lut_paths, invalid_lut_messages) = apply_lut_error_strategy(
        request.lut_error_strategy,
        valid_lut_paths,
    ...
    tokio::spawn(async move {
            ...
            .await
        {
            ...
            Err(e) => {
                logger::log_error(&format!(
                    "Failed to execute FFmpeg for task {}: {}",
                    task_id_clone, e
                ));
    ...
    match task_manager.cancel_task(&task_id) {
        Ok(_) => {
            ...
            if let Err(e) = video_processor.cancel_task(&task_id).await {
                logger::log_error(&format!(
                    "Failed to cancel processor task {}: {}",
                    task_id, e
                ));
            }
...
pub async fn get_video_info(
    ...
    let manager = if let Some(ffmpeg_path) = cfg_path {
        ...
        crate::core::video::VideoManager::with_paths(
            ffmpeg_path,
    ...
    if let Err(error) = upsert_video_info(&db, &info) {
        logger::log_warn(&format!(
            "Failed to persist video info for {}: {}",
            path, error
        ));
    }
