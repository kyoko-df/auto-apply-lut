...
impl FileUtils {
    ...
    fn group_files_by_hash(
        files: &[PathBuf],
    ) -> AppResult<std::collections::HashMap<String, Vec<PathBuf>>> {
    ...
    /// 清理临时文件
    pub fn cleanup_temp_files<P: AsRef<Path>>(
        directory: P,
        max_age_hours: u64,
    ) -> AppResult<usize> {
    ...
    pub fn create_backup<P: AsRef<Path>>(original_path: P) -> AppResult<PathBuf> {
        ...
        fs::copy(original_path, &backup_path).map_err(|e| {
            AppError::Io(format!(
...
impl BatchFileOperations {
    ...
    /// 批量移动文件
    pub fn batch_move(files: Vec<PathBuf>, destination: &Path) -> FileOperationResult {
        let mut processed = 0;
        let mut errors = Vec::new();
    ...
    pub fn batch_delete(files: Vec<PathBuf>, confirm: bool) -> FileOperationResult {
        ...
        for file in files {
            ...
            match result {
                Ok(_) => processed += 1,
                Err(e) => errors.push(format!("Failed to delete {}: {}", file.display(), e)),
            }
        }
