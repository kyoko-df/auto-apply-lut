...
impl MetadataExtractor {
    ...
    pub fn new() -> Self {
        Self {
            video_extensions: vec![
                ...
                "ts".to_string(), "mts".to_string(), "m2ts".to_string(),
            ],
            lut_extensions: vec![
                "cube".to_string(), "3dl".to_string(), "lut".to_string(),
                "csp".to_string(), "mga".to_string(), "m3d".to_string(),
            ],
            image_extensions: vec![
                "jpg".to_string(), "jpeg".to_string(), "png".to_string(),
                "gif".to_string(), "bmp".to_string(), "tiff".to_string(),
                "webp".to_string(), "svg".to_string(), "ico".to_string(),
            ...
            archive_extensions: vec![
                ...
                "xz".to_string(), "dmg".to_string(), "iso".to_string(),
            ],
            executable_extensions: vec![
                "exe".to_string(), "msi".to_string(), "app".to_string(),
                "deb".to_string(), "rpm".to_string(), "pkg".to_string(),
                "run".to_string(), "bin".to_string(),
            ],
        }
    }
    ...
    pub async fn extract_metadata<P: AsRef<Path>>(
        ...
        let hash = if options.calculate_hash && metadata.is_file() {
            Some(self.calculate_file_hash(path, &options.hash_algorithm).await?)
        } else {
            None
        };
...
mod tests {
    ...
    async fn test_metadata_extraction() {
        ...
        let metadata = extractor.extract_metadata(&test_file, options).await.unwrap();
    ...
    async fn test_file_type_detection() {
        ...
        assert_eq!(extractor.determine_file_type(&Some("mp4".to_string()), false), FileType::Video);
        assert_eq!(extractor.determine_file_type(&Some("cube".to_string()), false), FileType::Lut);
        assert_eq!(extractor.determine_file_type(&Some("jpg".to_string()), false), FileType::Image);
        assert_eq!(extractor.determine_file_type(&None, true), FileType::Directory);
        assert_eq!(extractor.determine_file_type(&Some("unknown".to_string()), false), FileType::Unknown);
    }
    ...
    async fn test_batch_extraction() {
        ...
        let options = MetadataOptions::default();
        let results = extractor.extract_metadata_batch(vec![&file1, &file2], options).await;
