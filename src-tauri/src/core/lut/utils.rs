...
impl LutUtils {
    ...
    pub fn detect_format_from_content(content: &str) -> Option<LutFormat> {
        ...
        if lines
            .iter()
            .any(|line| line.trim().starts_with("LUT_3D_SIZE"))
        {
        ...
        if lines
            .iter()
            .any(|line| line.trim().starts_with("CSPLUTV100"))
        {
    ...
    pub fn generate_identity_lut(
        ...
        match lut_type {
            ...
            LutType::Unknown => Err(AppError::InvalidInput(
                "Cannot generate identity LUT for unknown type".to_string(),
            )),
        }
    }

    /// 插值两个LUT
    pub fn interpolate_luts(lut1: &LutData, lut2: &LutData, factor: f32) -> AppResult<LutData> {
        ...
        match lut1.lut_type {
            ...
            LutType::Unknown => Err(AppError::InvalidInput(
                "Cannot interpolate unknown LUT type".to_string(),
            )),
        }
    }
    ...
    pub fn invert_lut(lut: &LutData) -> AppResult<LutData> {
        match lut.lut_type {
            ...
            LutType::Unknown => Err(AppError::InvalidInput(
                "Cannot invert unknown LUT type".to_string(),
            )),
        }
    }
    ...
    pub fn adjust_lut_intensity(lut: &LutData, intensity: f32) -> AppResult<LutData> {
        ...
        match lut.lut_type {
            LutType::ThreeDimensional => {
                ...
                // 生成身份LUT用于混合
                let identity =
                    Self::generate_identity_lut(lut.size, lut.lut_type, lut.format.clone())?;
            ...
            LutType::Unknown => Err(AppError::InvalidInput(
                "Cannot adjust intensity for unknown LUT type".to_string(),
            )),
        }
    }
